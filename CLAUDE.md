...
### Two-Step Lecture Authoring

Creating a lesson and giving it content are separate calls. Step 1 — `POST sections/<id>/contents/` `{item_type: 'lecture', title}` — creates the row alone (`lecture_type` is optional and defaults to `video`, no payload required). Step 2 — `PATCH lectures/<id>/` with `{lecture_type, article_content}` or multipart `{lecture_type: 'video', video_file}` — makes it playable. No new endpoint, **no migration**: a step-1 lecture is "a video lecture whose video hasn't arrived", a state the schema already allowed.

**"Awaiting content" is derived, never stored** — `Lecture.is_awaiting_content` (one row; reads a prefetch when present) and `lectures_awaiting_content(queryset)` (bulk), both in `content_models.py`: `lecture_type='video'` **and** no active `VideoAsset`. `Quiz`, `Assignment` and `CodingExercise` carry the same-named property (no questions / no questions / no code in any of the three code fields), all exposed on `SectionContentSerializer` so the builder can tell a shell from authored work. **The flag means "replacing this would lose nothing", NOT "this is finished"** — an assignment with one ungradable question still blocks submission but is authored, so it is not awaiting content. The AI re-apply deletes shells; conflating the two would delete real work. Do not add an `is_placeholder` column — a stored flag drifts (a failed transcode would leave it claiming content), while `VideoAsset.status` is already authoritative. **Article lectures can never be awaiting content**: `chk_lecture_payload_by_type` forbids a blank body for that type.

**Every assignment must be gradable** — `_validate_course_completeness` blocks an assignment with no questions, or a question with neither `model_answer` nor `rubric`. Such a question grades every submission to 0 (`RubricGrader` short-circuits on an empty `rubric_snapshot`, and `_autofill_rubric` only derives a rubric when a model answer exists), and nothing used to check it, so an unfinished assignment reached learners and failed all of them silently. Reported under its own `assignments` error key, like `quizzes` and `coding_exercises`.

An empty lecture must not behave like content, enforced at exactly four places — `_validate_course_completeness` (`empty_lectures`, blocks leaving draft), `load_learner_curriculum` (hidden from learners, **still shown to instructors** in preview), `recalculate_progress` (out of `total_items` — otherwise 100% is unreachable and the certificate never issues), and `load_catalog_curriculum` (pruned, which fixes both item counts). `empty_lectures` and `video_processing` are disjoint: "never uploaded" vs "uploaded, not ready". `content_outline_stats()` is deliberately unchanged — `empty_lectures` already blocks the course, and two errors for one problem is worse. `LectureDetailAPIView._guard_section` passes `None` for an awaiting lecture so a lesson created before a cohort section unlocked can still be filled in. See `docs/architecture/33-two-step-lecture-authoring.md`.

### Content Ownership: Creator-Inclusive

Authoring endpoints resolve ownership as **assigned instructor OR course creator**, never instructors-only. A partner institution owns its courses through `created_by` and is never added to `course.instructors` (only its experts are), so an instructors-only filter locks the institution out of its own content — it could create sections but 404'd on every content create, leaving the course unsubmittable.

One helper, `courses/utils.py` → `course_owner_q(user, path='course')` (`Q(instructors) | Q(created_by)`), plus `owned_course_qs` / `owned_section_qs`. **Always pair `course_owner_q` with `.distinct()`** — the M2M join duplicates rows for a user who is both instructor and creator.

**On a queryset carrying an aggregate, filter by subquery instead of joining.** The assignment views/service annotate `Sum('questions__points')`; an ownership join over a multi-valued relation multiplies that sum, and `.distinct()` does **not** fix it. They use `section__in=_owned_section_ids(user)` → `WHERE section_id IN (SELECT ...)`.

Authoring gates are `IsCourseCreator`, not `IsInstructorUser` (`content_views.py`, `coding_views.py`, `assignment_views.py`).
...
**`courses/services/ai_outline_service.py`** is pure HTTP transport, shaped like `payments/services/sslcommerz_service.py`: `REQUEST_TIMEOUT = (5, 45)` (long read leg — it's an LLM call; the AI service's own LLM timeout sits at 40s so it fails first with a real status. **Deployment:** gunicorn's default `--timeout` is 30s, below this — raise it to 60s). **The AI service's `LLM_MAX_OUTPUT_TOKENS` is bounded by the Groq account's tokens-per-minute limit, not the model's output ceiling** — Groq charges `max_completion_tokens` against TPM before generating, so a cap above the tier's allowance fails every request with `413 rate_limit_exceeded`. `MAX_MODULES`/`MAX_PLAN_ITEMS` are sized to fit inside that budget; raising them without raising the tier turns large courses into 502s., `AIOutlineError(message, http_status=503)` in the `ScheduleError`/`ReviewError` mould. **Every upstream failure collapses to one 503 with one generic message** — network error, key mismatch, provider 502, unparseable body are indistinguishable to the client. The reason is logged; never forward it.
