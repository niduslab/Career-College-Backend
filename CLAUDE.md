...
**Shared login opens the admin session too.** The common `POST /api/v1/auth/login/` (`UserLoginView`) is the single login for every role and still returns JWT; when the user is an admin (`is_staff or user_type == 'admin'`) it *additionally* calls `django_login`, stamps `session['admin_login_at']`, and primes `csrftoken` via `get_token()` — one login yields JWT **plus** `sessionid` + `csrftoken`, so the back-office needs no separate admin-console login. Non-admins unaffected. `UserLoginView` is `AllowAny` with no `SessionAuthentication`, so this doesn't force CSRF on the login POST. The dedicated admin-console `auth/login/` + `auth/csrf/` views below still work but are no longer the only entry.

Endpoints: `GET auth/csrf/` (primes `csrftoken`), `POST auth/login/` (`AdminLoginView`, `AllowAny`+no auth classes+throttled → `django.contrib.auth.login`, stamps `session['admin_login_at']`), `POST auth/logout/` (CSRF-protected), `GET auth/session/` (who-am-I). `AdminLoginSerializer` mirrors `UserLoginSerializer` account-state checks; the admin-role gate runs **in the view** so bad creds → 400 generic (no enumeration) but valid-non-admin → **403**. Idle timeout = `SESSION_SAVE_EVERY_REQUEST=True` + `SESSION_COOKIE_AGE=ADMIN_SESSION_IDLE_TIMEOUT` (sliding). Sensitive-action re-auth via `IsRecentlyAuthenticatedAdmin`. 2FA deferred (no lib). See `docs/architecture/24-admin-console-auth.md` and `docs/api-testing/postman-admin-console.md`.
