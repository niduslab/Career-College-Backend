...
**Drip release:** `CourseSection.unlocks_at` (nullable; NULL = released) gates learners section-level. `guard_editable(course)` (`courses/utils.py`) has one narrow carve-out: a `published` course with an **ongoing** schedule stays content-editable (weekly uploads mid-run); closes again at `completed`. No admin re-review for drip additions — `AuthoredModel` stamping is the audit trail. Section create/update serializers accept `unlocks_at`.
...
Endpoints: `POST checkout/` (learner-gated; body: **exactly one** of `{course_slug}` / `{webinar_slug}` → `gateway_url` + `item_type`) · `POST ipn/`, `GET/POST success|fail|cancel/` (**`AllowAny` + `authentication_classes=[]`** — gateway posts carry no JWT; the success redirect runs finalize and 302s to `FRONTEND_URL + FRONTEND_PAYMENT_*_PATH`) · `GET orders/[<pk>/]` (own-only; numeric id → 404 on no-access; `gateway_payload`/`val_id` never serialized). The **success redirect is the primary finalize path** (local sandbox has no public IPN URL); IPN is the production safety net.
