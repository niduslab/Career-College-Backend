...
| `admin_console` | `/api/v1/admin-console/` | Platform back-office: **session-based admin auth**, device/session tracking, user management + audit log, and the `PlatformSettings` singleton (branding + default certificate signatory). Base view (`AdminConsoleAPIView`) for all admin endpoints. |
...
**Three audience segments share the app, split by URL prefix: `instructor/`, `partner/`, `admin/`.** They do **not** share service modules — `instructor_summary` / `institution_summary` / `platform_summary` are separate functions with separate scoping rules. Never merge them into one conditional function; the scope resolution is the whole point of the split.

#### Individual instructor surface (`instructor/`)

For a **single instructor's own courses**, gated `[IsAuthenticated, IsEmailVerified, IsInstructorUser]` — the **unverified** variant, matching how `IsCourseCreator` (not `IsVerifiedCourseCreator`) gates ordinary authoring. Day-to-day dashboard/roster viewing must not require completed identity verification. Scope is resolved from `request.user` directly (an instructor has no institution profile to key off) via the dual-path filter `Q(instructors=user) | Q(created_by=user)` with a **mandatory `.distinct()`** — a course where the user is both a roster member and `created_by` would otherwise double-count.

| Endpoint | View | Purpose |
|---|---|---|
| `GET /api/v1/analytics/instructor/summary/` | `InstructorAnalyticsSummaryView` | Merged dashboard KPI payload: revenue (gross/paid_orders/growth), students (total/active/growth), courses (total/published/draft + status breakdown), rating (weighted avg), completion funnel (enrolled→started→completed), top 5 courses. Fixed ~12 queries. |
| `GET /api/v1/analytics/instructor/students/` | `InstructorStudentListView` | Paginated student roster. `?search=` (≥2 chars, name/email), `?course_id=`, `?status=active\|inactive\|completed\|not_started\|unenrolled`, `?sort=` (whitelist). |
| `GET /api/v1/analytics/instructor/students/summary/` | `InstructorStudentSummaryView` | Roster-wide KPIs + course-filter options. Separate from the list because it describes **every** student, not the current page. |
| `GET /api/v1/analytics/instructor/revenue/summary/` | `InstructorRevenueSummaryView` | Gross revenue cards + per-course breakdown + trend series. `?granularity=monthly\|weekly&periods=N`. |
| `GET /api/v1/analytics/instructor/revenue/orders/` | `InstructorOrderListView` | Paginated paid-order history. `?course_id=`, `?sort=-paid_at\|paid_at\|-amount\|amount`. |

Services: `analytics/services/instructor_analytics_service.py` (summary), `analytics/services/instructor_students_service.py` (roster), and `analytics/services/instructor_revenue_service.py` (revenue). All three are imported by **full module path** — the flat `analytics.services` re-export stays partner-only to avoid name clashes, same rule as the admin service.

**Revenue is gross-only, deliberately** (`docs/architecture/31-instructor-revenue.md`): sum of `Order.amount` where `status='paid'`, scoped by the same dual-path course filter. No payout, "available balance", commission split, or bank account — none of those have a backing model, matching the Payments section's "Phase 2 — not built" note. `Order.Status` has no `REFUNDED` state; a duplicate payment is marked `FAILED` with a `gateway_payload.requires_refund` audit flag (manual, off-system), never netted out of revenue. The trend series reuses `analytics_service.build_value_series()` bit-for-bit with the admin revenue trend (same `Sum('amount')` bucketed on `created_at`), just scoped to one instructor's courses instead of the whole platform.

**The roster's one derived field is `status`**, computed from three real columns in `derive_status()`, with an exact SQL twin in `_status_filter_q()`. Filtering happens in SQL, never by filtering a serialized page — otherwise the paginator's `count` would describe a different set than the rows returned. The two functions must be changed together or they drift. Precedence is `completed → unenrolled → not_started → inactive → active`: a learner who finished and then stopped opening the course is `completed`, not `inactive`. `INACTIVE_AFTER_DAYS = 14` is a **product threshold, not a measurement** — echoed back to the client as `inactive_after_days` so no frontend hardcodes its own copy.

**One roster row = one enrollment, not one learner.** A learner in three of the instructor's courses is three rows with three progress values, because the instructor's question is per-course. The summary reports **distinct learners** in `total_students`/`active_students` and **enrollment rows** in `status_breakdown` — the two deliberately don't sum to the same number, and the field names say which is which. Never "fix" this by making them agree.

**`last_active_at` is `Enrollment.last_accessed_at` (course-scoped), never `LearnerActivityDay`.** The activity ledger is platform-wide and day-granular — using it would show a learner as active today because they studied someone *else's* course. Nullable: a learner who enrolled and never opened content is `null`, which renders as "Never", not as a date.

**Honesty rule, same as the learner dashboard.** `docs/architecture/29-instructor-dashboard-analytics.md` and `30-instructor-students.md` each carry a "deliberately absent" section listing what was dropped and why: no watch-time trend chart (`WatchProgress.last_watched_at` is `auto_now` — it overwrites, so no history survives; a trend needs a new append-only ledger first), no traffic-source donut (nothing records referrers), no AI insights, and no "engagement tips" carrying invented statistics. Do not add any of these without a real backing model.

#### Partner institution surface (`partner/`)
...
`Certificate` is auto-issued when `recalculate_progress()` transitions an enrollment to 100% for the first time. The issuance callback fires via `transaction.on_commit(lambda: _issue_certificate_and_notify(enrollment.pk))` in `enrollment_service.py`. `issue_certificate()` is idempotent (returns the existing row unchanged). PDF rendered on-the-fly by reportlab (`courses/certificate_pdf.py`) — no file stored on disk. See `docs/architecture/14-certificate-system.md` and `docs/api-testing/postman-certificates.md`.

**Two public identifiers.** `certificate_uid` (UUID4, non-guessable) backs the share/download links; `certificate_id` (`CC-<YYYY>-<SLUG-ABBREV>-<NNNNNN>`, e.g. `CC-2026-NEXTJS-000123`) is the human-readable credential printed on the certificate. Generated in `_next_certificate_id` by counting existing IDs on the same `(year, abbrev)` prefix; the **unique constraint is the real race guard** — `issue_certificate` retries up to 5× on `IntegrityError`. **Permanent: written only in the create path, never regenerated, never derived from the pk.**

**Everything on the certificate is a frozen snapshot** — learner name, course title, completion date, duration, learning hours, both signatories' names/designations, **and copies of both signature image files**. The signature images are **copied into `media/certificates/signatures/` at issuance, not referenced**: an FK or stored path points at a file that `ImageField.save()` on the source profile would overwrite, silently rewriting every past certificate. Copying is the only construction that actually freezes it (requirements §5). Copies read through the `FieldFile`, never `.path()` (S3-safe), and each is wrapped in its own try/except → `logger.warning` — **a failed signature copy must never cost the learner their certificate**. Regression: `test_later_signature_change_does_not_alter_issued_certificate`.

**Signatory fallback chain** (`_resolve_authorized_signatory`): institution-owned course with a configured `PartnerInstitutionProfile.authorized_signatory_name` → that institution's signatory (issuer = institution name); otherwise → the `admin_console.PlatformSettings` singleton (issuer = its `organization_name`, or the institution name when the course is institution-owned but has no signatory of its own). Instructor designation comes from `InstructorProfile.current_title` → `headline` → `'Course Instructor'`. **Nothing is required** — with no signatory configured, issuance still succeeds with blank fields and the PDF omits the block.

**Signature uploads reuse existing endpoints — no new upload endpoint, no new permission class.** `InstructorProfile.signature` and `PartnerInstitutionProfile.authorized_signatory_*` ride `PATCH /api/v1/auth/profile/me/` (both profile serializers use `exclude = ['user']`, so new fields are exposed automatically); the platform default is `PATCH /api/v1/admin-console/platform-settings/`. `core/validators.py:validate_signature_size` (2 MB) is the project's first file-size validator.

**Eligibility:** `issue_certificate` raises `CertificateError(422)` when `enrollment.completed_at is None`. The only production caller already gates on completion — the guard stops any *future* caller from bypassing it. `CertificateError(message, http_status)` mirrors `ScheduleError`/`ReviewError`.

**Status + revocation.** `status` is `valid|revoked`; `revoke_certificate`/`restore_certificate` (`IsPlatformAdmin`) take `select_for_update` and write an `AdminActionLog` row in the same transaction. **Revocation changes only the verdict, never the issued snapshot.** Double-revoke / restoring a valid certificate → 422.

**Endpoints** (`/api/v1/courses/`): `GET certificates/verify/<str:identifier>/` (public; accepts **either** the certificate ID or the UUID — declared *after* the `<uuid:...>` routes so the typed converter wins), plus the original `certificates/<uuid>/verify/` and `/download/`, `POST certificates/<uuid>/revoke/|restore/`, and `GET admin/certificates/` (`IsPlatformAdmin`; `?search=` ID/learner/course >= 2 chars, `?status=`, whitelisted `?sort=`) — the **discovery surface** for revoke/restore, which otherwise need a UUID an admin cannot look up; an unknown `sort` falls back to newest-first rather than 400ing, since it is a browse screen and an unordered queryset breaks pagination. A **revoked certificate verifies as 200 with `status: revoked`**, not 404 — "exists but revoked" is the answer a verifier needs; an unknown identifier is 404 with an identical message either way.

**`build_verification_url(certificate)`** is the single URL builder, used by both the serializers and the PDF so the printed URL, API payload, and QR code can never disagree: `FRONTEND_URL + CERTIFICATE_VERIFY_PATH + certificate_id`. **`FRONTEND_URL` must be the production domain in production** — requirement §7's "no localhost" is an environment concern, not a runtime check, and a localhost value is printed verbatim onto every PDF issued while it is set. Note the two deliberately distinct fields: `verify_url` (relative API path) vs `verification_url` (absolute frontend URL, what the QR encodes). **The QR is rendered in both places from that one value** — `_draw_qr()` in `certificate_pdf.py` (the `qrcode` lib, pure Python, best-effort: a failure logs a warning and the PDF still renders, since the URL is printed as text beside it) and `qrcode.react` on the frontend's verify page. The PDF needs its own because that is the artifact people print and attach to a CV.

**Analytics counted-vs-trend split — do not "fix" the apparent inconsistency.** Summary "certificates earned" counts, the funnel's `certified` stage, expert-performance counts, and the instructor roster's `has_certificate` all filter `status=VALID` (a revoked certificate is not an earned credential). The **issuance trend series stay unfiltered** — the certificate genuinely was issued that month, and the trend is a historical record.

**Historical rows keep blank snapshot fields.** Migration `courses/0004` backfills only `certificate_id` (deterministic, ordered by `issued_at, id`); filling the signatory fields from today's profiles would fabricate a snapshot that was never true.

`NidusCourse.learning_hours` (instructor-declared, snapshotted onto the certificate) is **separate from `duration_minutes`** (raw video runtime, catalog-facing) — do not conflate them.
...
### AI-Assisted Authoring (`courses/all_views/ai_views.py`, `/api/v1/courses/ai/`)

LLM-drafted content for the course builder, calling out to the **`Career-College-AI-Services` FastAPI project** (a separate repo hosting every AI feature). Topology is one-directional: frontend → Django (JWT) → AI service (`X-Service-Key`) → Groq. **The frontend never calls the AI service; the AI service never touches this database; `GROQ_API_KEY` never exists in this repo.** Django↔AI is server-to-server with a shared secret, not JWT — Django has already authorized the caller before the hop. See `docs/architecture/32-ai-course-outline-generator.md` and `docs/api-testing/postman-ai-course-outline.md`.

**First and only endpoint: `POST ai/outline-preview/`** (`CourseOutlinePreviewAPIView`, `[IsAuthenticated, IsEmailVerified, IsCourseCreator]`). Body = course metadata (`title`/`description`/`audience` required; `prerequisites`/`level`/`language`/`duration_minutes`/`category`/`extra_instructions` optional); returns `{modules: [...], outline_text: str}`. `IsCourseCreator` (not `IsInstructorUser`, unlike the older rubric preview) so **partner institutions can use it too**, and not the `Verified` variant so it works before identity verification — same gate as `CourseCreateAPIView`.

**It persists nothing.** Same rule as `AssignmentRubricPreviewAPIView`: generate a suggestion, return it, let the human decide. The caller applies the draft through endpoints that already exist — one `CourseSection` per module via `POST <pk>/sections/create/` (the frontend's path), and/or the edited `outline_text` into `course_outline` via the normal course create/update. **Never make this endpoint write.** The frontend tracks the section ids its last apply produced and `PATCH`es those on a second apply (regenerate → apply again **updates in place**, it does not stack a second batch); nothing is ever deleted, so a reused section keeps its lessons and a shrunken outline leaves its leftovers for the user to remove deliberately. That provenance **cannot be queried** — `created_by`/`created_at` are identical for AI-applied and hand-made sections because Django stamps `request.user` on both, and `created_at` clustering is a heuristic that would eventually overwrite hand-authored content — so it is kept in `localStorage` per course (a UI hint, like `session.ts`'s login flag), and when there is no record but sections exist the UI **asks** rather than guessing. Do not add a `created_at`-based guess; if this ever needs to be authoritative it needs a provenance column on `CourseSection` and a migration. `learning_outcomes` are deliberately *not* pushed into `NidusCourse.learning_objectives` — that would overwrite instructor-typed content.

**`outline_text` must never contain blank lines.** `NidusCourseCreateUpdateSerializer._normalize_multiline` strips them on save, so any layout depending on blank lines would silently change once persisted. The AI service renders the text in Python from `modules` (the model is asked for `modules` only) precisely so the preview and the saved value cannot diverge.

**`AIOutlineThrottle`** (`scope='ai_outline'`, `rate=AI_OUTLINE_RATE_LIMIT`, default `10/min`, defined in the view module like every other custom throttle here) is the **only throttle in the project that guards spend rather than data integrity** — every call is a paid LLM request taking seconds. Any future AI endpoint needs its own throttle for the same reason.

**`courses/services/ai_outline_service.py`** is pure HTTP transport, shaped like `payments/services/sslcommerz_service.py`: `REQUEST_TIMEOUT = (5, 45)` (long read leg — it's an LLM call; the AI service's own LLM timeout sits below 45s so it fails first with a real status), `AIOutlineError(message, http_status=503)` in the `ScheduleError`/`ReviewError` mould. **Every upstream failure collapses to one 503 with one generic message** — network error, key mismatch, provider 502, unparseable body are indistinguishable to the client. The reason is logged; never forward it.

Env: `AI_SERVICES_BASE_URL`, `AI_SERVICES_KEY` (these two cover **every** AI feature — the next one adds only a URL path, no new var), `AI_OUTLINE_RATE_LIMIT`. All have defaults, so CI is unaffected. The AI service must be reachable only from this backend's network.
...
**Admin content preview:** `GET /<pk>/review/curriculum/` (`CourseAdminCurriculumView`, `IsPlatformAdmin`) returns the full curriculum tree at instructor depth — video/article content, quiz correct answers, assignment model answers/rubric, coding solution code + evaluation script — so the admin can judge actual content before approving, not just the base-fields + outline-stats that `GET /<pk>/review/` returns. Loader is `curriculum_service.load_admin_review_curriculum()`, a third audience-specific bulk-loader alongside the catalog and learner ones — never reuse the learner-safe or catalog serializers for it, and never expose it outside `IsPlatformAdmin`.
...
Platform back-office. First slice is a **session-based admin login**, separate from the JWT flow — a browser back-office wants short idle timeouts + CSRF-protected writes, which don't fit the 12 h JWT. JSON API (SPA renders the page). Owns three models — `AdminSession` (device/session tracking), `AdminActionLog` (audit trail), and `PlatformSettings` (see below); admin auth itself still rides the existing `django_session` table.
...
**Platform settings singleton.** `PlatformSettings` (`admin_console/all_models/platform_settings_models.py`, `db_table='platform_settings'`) holds platform branding (`organization_name`) and the **default certificate authorized signatory** (name / designation / signature image). Always `pk=1` — `save()` pins it, `delete()` is a no-op, and **`PlatformSettings.load()` is the only read path** (never `objects.get()`). `GET/PATCH /api/v1/admin-console/platform-settings/` (`PlatformSettingsView`, subclasses `AdminConsoleAPIView`, multipart-capable) writes a `PLATFORM_SETTINGS_UPDATE` audit row on PATCH. It lives here rather than in `core/` because `core` has no `models.py` and is not an installed model app. Consumed by `certificate_service._resolve_authorized_signatory` as the fallback when a course has no institution signatory — see *Certificate System*. `AdminActionLog.target_user` is **nullable** and is `None` for settings changes (and carries the learner for certificate revocations, whose subject is in `metadata`).
