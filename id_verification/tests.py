...
class ExpertManagementTests(PartnerInstitutionTestBase):
    ...
    def test_provision_expert_creates_affiliated_instructor(self):
        with patch('authentication.tasks.send_expert_credentials_email_task.delay') as mock_mail, \
        ...
        # Response exposes both PKs: profile id and the User id (for roster assignment).
        self.assertEqual(resp.data['data']['user_id'], expert.id)
        self.assertEqual(resp.data['data']['id'], expert.instructor_profile.id)
    ...
    def test_deactivate_expert_blocks_authoring(self):
        with patch('authentication.tasks.send_expert_credentials_email_task.delay'), \
             patch('authentication.services.expert_service.transaction.on_commit',
                   side_effect=lambda fn: fn()), \
             patch('notifications.services.dispatcher.dispatch'):
            self.client.post(
